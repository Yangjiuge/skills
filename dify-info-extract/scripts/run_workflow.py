#!/usr/bin/env python3
"""Upload local files to Dify and run the target workflow."""
...
DEFAULT_USER = "codex-dify-info-extract"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--scene",
        help=(
            "Optional text value for the workflow's scene input. When provided, "
            "it is sent as inputs.scene."
        ),
    )
    parser.add_argument(
        "--inputs-json",
...
def main() -> None:
    ...
    set_text_input(inputs, "scene", args.scene)
